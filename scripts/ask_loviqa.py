...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Path to the video file (used to derive video_name and duration). Optional if you use --video_name and only query Milvus.",
    )
    parser.add_argument(
        "--video_name",
        type=str,
        required=False,
        help="Video name used to locate Milvus collection `videoqa_<video_name>`. Useful on machines without the original .mp4.",
    )
    parser.add_argument(
        "--video_duration",
        type=float,
        required=False,
        help="Optional duration in seconds (only used for prompt metadata when --video is not provided).",
    )
    ...
    if args.video and (not os.path.exists(args.video)):
        if not args.video_name:
            ...
            return
        print(
            f"⚠️ Video file not found: {args.video}. Proceeding with --video_name={args.video_name} (Milvus-only query)."
        )
